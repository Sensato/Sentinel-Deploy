...
echo "gvmd should be configuring nvt(the latest vulernablity feeds) in the meanttime"
...
echo "Please wait for one minute then run run ps aux | grep -E 'openvassd|gsad|gvmd' | grep -v grep after a restart"
