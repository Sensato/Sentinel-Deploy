...
echo -e "Please open a new terminal with ${Yellow}CTRL+ALT+T${White}, then run: ${Green}sudo apt install python3-pip${White} to install pip3,"
echo -e "and ${Green}pip3 install --upgrade pip${White} to upgrade pip3"
echo -e "Then, please run ${Green}pip3 install --user gvm-tools${Yellow} in any terminal except the one you used to upgrade pip3."
...
echo -e "Please wait for one minute then run ${Green}ps aux | grep -E 'openvassd|gsad|gvmd' | grep -v grep ${Yellow}again after a restart."
...
echo -e "${White}It will automatically create a target, use the target for a task, and start a task."
